"""
Qogita Health Category Monitor — Webhook-driven version.

This script is invoked by a GitHub Actions workflow that triggers on
`repository_dispatch` (event_type: qogita_catalog_health), fired by a
Cloudflare Worker when Qogita's async catalog-download job completes.
...
CSV columns (confirmed from a real Makeup category download; same shape for Health):
...
SNAPSHOT_FILE    = "snapshot_qogita_health_category.json"
BASELINE_FLAG    = "baseline_done_health_category.txt"
...
def notify_new(product):
    ...
    embed = {
        ...
        "footer":    {"text": f"Qogita {CATEGORY_NAME} Category Monitor • qogita.com"},
    }
    _send_embed(embed)
...
def notify_download_failed():
    embed = {
        "title":     f"⚠️  Catalog download failed — {CATEGORY_NAME}",
        "color":     COLOUR_FAILED,
...
def check_changes(product, old):
    ...
    for key in ("barcode", "brand", "category"):
...
def main():
    print("=" * 55)
    print(f"  Qogita {CATEGORY_NAME} Category Monitor (webhook-driven)")
    print("=" * 55)
