"""
L'Oréal Brand Monitor — Webhook-driven version.

This script is invoked by a GitHub Actions workflow that triggers on
`repository_dispatch` (event_type: qogita_catalog_loreal), fired by a
Cloudflare Worker when Qogita's async catalog-download job (filtered
by brand_names=["L'Oréal"]) completes.
...
BRAND_DISPLAY_NAME = "L'Oréal"
SNAPSHOT_FILE    = "snapshot_qogita_loreal_brand.json"
BASELINE_FLAG    = "baseline_done_loreal_brand.txt"
...
DISCORD_WEBHOOK  = os.getenv("DISCORD_WEBHOOK_LOREAL", "")
...
def main():
    ...
    if not DISCORD_WEBHOOK:
        print("  [!] DISCORD_WEBHOOK_LOREAL must be set")
        sys.exit(1)
