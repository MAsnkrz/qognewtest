"""
Mustela Brand Monitor — Webhook-driven version.

This script is invoked by a GitHub Actions workflow that triggers on
`repository_dispatch` (event_type: qogita_catalog_mustela), fired by a
Cloudflare Worker when Qogita's async catalog-download job (filtered
by brand_names=["Maybelline"]) completes.
...
BRAND_DISPLAY_NAME = "Mustela"
SNAPSHOT_FILE    = "snapshot_qogita_mustela_brand.json"
BASELINE_FLAG    = "baseline_done_mustela_brand.txt"
...
DISCORD_WEBHOOK  = os.getenv("DISCORD_WEBHOOK_MUSTELA", "")
